...
class HaystackApi(protorpc.remote.Service):
    ...
    def dbevent_insert(self, request):
        if request.from_datastore:
            ...
            my_quote = DbEvent(title=request.title, address=request.address, from_date_time=request.from_date_time, to_date_time=request.to_date_time, category=request.category, description=request.description, comments=request.comments, comment_size=request.comment_size, lat=request.lat, lon=request.lon, likes=request.likes, likes_size=request.likes_size)
        my_quote.put()
        return my_quote
    ...
    @DbEvent.method(request_fields=("entityKey",), response_fields=("comments", "likes"), name="dbevent.one", path="haystack/dbevent/one/{entityKey}", http_method="GET")
