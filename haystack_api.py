...
class HaystackApi(protorpc.remote.Service):
    ...
    def dbevent_insert(self, request):
        if request.from_datastore:
            ...
            my_quote = DbEvent(title=request.title, address=request.address, from_date_time=request.from_date_time, to_date_time=request.to_date_time, category=request.category, description=request.description, comments=request.comments)
        my_quote.put()
        return my_quote
